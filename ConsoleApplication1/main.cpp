...
int main(void)
{
	...
	try
	{
		...
		while (ERROR_INPUT)
		{
			std::cout << "Input '6' or '9' to choose 6 axis or 9 axis\n - 6: 6 axis\n - 9: 9 axis" << std::endl;
			char str;
			std::cin >> str;
			if (str == '6')
			{
		...
		while (1)
		{
			...
			if (oneByte[0] == 0x0a && lastByte == 0x0d)
			{
				...
				//continue;
				if (NINE_AXIS_ENABLE == 1 && Calibrated == 0)
				{
					double com_tmp = 0;
					for (int i = 0; i < 3; i++)
					{
				...
				else if (SIX_AXIS_ENABLE == 1 && Calibrated == 0)
				{
					Calibrated = 1;
				}
...
void stepDetection()
{
	if (Calibrated == 1 && step_detection_begin == 1)
	{
		...
		double AccX_Y = 0 - acc1[star1 - 1][1] * GRAVITY;       //Y
		...
		if (abs(AccY_Z) >= abs(AccX_Y) && abs(AccY_Z)>abs(AccZ_X))
		{
			...
			double GyroX_Z = gyro1[star1 - 1][0] - GyroX_e0;
